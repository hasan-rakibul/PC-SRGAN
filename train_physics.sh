...
--config_path ./configs/train/Allen-Cahn_Periodic_Physics.yaml >&1 | tee ./bash_logs/Allen_Cahn_Periodic_Physics_$(date +%y%m%d-%H%M).txt
    ...
    echo "Training Allen-Cahn Periodic_x4"
    python src/SRGAN_train.py \
--config_path ./configs/train/Allen-Cahn_Periodic_Physics_x4.yaml >&1 | tee ./bash_logs/Allen_Cahn_Periodic_Physics_x4_$(date +%y%m%d-%H%M).txt
