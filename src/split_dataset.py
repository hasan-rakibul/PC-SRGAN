...
def split_and_move(src_mesh7, src_mesh_high, dst, dst_ratio=0.10, dst_split_ratio=0.05):
    ...
    assert 'train' in src_mesh7.split('/') and 'train' in src_mesh_high.split('/'), 'the source directories should contain "train" in their path'

    dst_mesh7 = src_mesh7.replace('train', dst)
    dst_mesh_high = src_mesh_high.replace('train', dst)

    if os.path.exists(dst_mesh7) or os.path.exists(dst_mesh_high):
        print('Test/validation directories already exist. Exiting...')
        return

    os.makedirs(dst_mesh7)
    print('Created directory: ', dst_mesh7)
    os.makedirs(dst_mesh_high)
    print('\tCreated directory: ', dst_mesh_high)
    
    folders = natsorted(os.listdir(src_mesh7))
    num_folders = len(folders)
    assert num_folders == len(natsorted(os.listdir(src_mesh_high))), 'Number of folders in mesh_7 and mesh_high are not equal'
    ...
    for folder in test_folders:
        ...
        src_path_high = os.path.join(src_mesh_high, folder)
        dst_path_high = os.path.join(dst_mesh_high, folder)
        shutil.move(src_path_high, dst_path_high)
        print('\tMoved folder (mesh_high): ', folder)
    ...
    for folder in test_split_folder:
        ...
        src_path_high = os.path.join(src_mesh_high, folder)
        dst_path_high = os.path.join(dst_mesh_high, folder)
        # the file names in mesh_high are the same as mesh_7
        for file in files[50:]:
            src_file = os.path.join(src_path_high, file)
            dst_file = os.path.join(dst_path_high, file)
            if not os.path.exists(dst_path_high):
                os.makedirs(dst_path_high)
                print('\tCreated directory: ', dst_path_high)
            shutil.move(src_file, dst_file)
            print('\tMoved file (mesh_high): ', file)

def main():
    ############################################

    src_mesh7 = './data/Allen-Cahn_Periodic_x4/train/mesh_7'
    src_mesh_high = './data/Allen-Cahn_Periodic_x4/train/mesh_31'
    ...
    split_and_move(src_mesh7, src_mesh_high, "validation", dst_ratio, dst_split_ratio)
    split_and_move(src_mesh7, src_mesh_high, "test", dst_ratio, dst_split_ratio)
