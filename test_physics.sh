...
--config_path ./configs/test/Allen-Cahn_Periodic_Physics.yaml >&1 | tee ./bash_logs/Allen_Cahn_Periodic_Physics_Test_$(date +%y%m%d-%H%M).txt
    ...
    echo "Testing Allen-Cahn Periodic x4"
    python src/SRGAN_test.py \
--config_path ./configs/test/Allen-Cahn_Periodic_Physics_x4.yaml >&1 | tee ./bash_logs/Allen_Cahn_Periodic_Physics_x4_Test_$(date +%y%m%d-%H%M).txt
